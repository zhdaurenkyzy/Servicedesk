        			...
        			<div class="row" align="left">
                      <c:forEach var="language" items="${languages}">
                        <form method="POST" action="/setLanguage">
                         <input id="idField" type="hidden" name="page" value ="userCabinet">
...
<form method="POST" action="${uri}">
     <p><fmt:message key = "key.userName" bundle = "${resourceBundle}"/>: <input id="nameField" type="text" name="name" required value = <c:out value="${user.getName()}"/>></p>
      ...
      <p><fmt:message key = "key.userLogin" bundle = "${resourceBundle}"/>:
        <c:if test="${uri == '/updateUser'}"> <c:out value="${user.getLogin()}" /></c:if>
        <c:if test="${uri == '/registration'}"> <input id="loginField" type="text" required name="login"/></c:if>
      </p>
      <p><fmt:message key = "key.password" bundle = "${resourceBundle}"/>: <input id="passwordField" type="password"  name="password" required></p>
       <c:if test="${uri == '/updateUser'}"><p><fmt:message key = "key.confirmPassword" bundle = "${resourceBundle}"/>: <input id="passwordField" type="password"  name="repeatPassword" required></p></c:if>
     <div class="row">
     <c:if test="${uri == '/updateUser'}">
         <input type="submit" value=<fmt:message key = "key.update" bundle = "${resourceBundle}"/>>
     </c:if>
     <c:if test="${uri == '/registration'}">
